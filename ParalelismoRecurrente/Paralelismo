...
func mainFibonacci(numero int64){
  ...
  go func() {
    defer wg.Done()
    fmt.Println("-----Fibonacci Recursivo-----(M1)")
    start := time.Now()
    //fmt.Println("Resultado Fib iterativo:", ())
    go fibonacciRecur(numero)
    end := time.Now()
    fmt.Printf("(M1) Calculation finished in %s \n",  end.Sub(start))  
  }()
  /*go func() {
    defer wg.Done()
    start := time.Now()
  ...
  go func() {
    defer wg.Done()
    start := time.Now()
    fmt.Println("-----Fibonacci Recursivo-----(M2)")
    //fmt.Println("Resultado Factorial recursivo:", )    
    go fibonacciRecur(numero)
    end := time.Now()
    fmt.Printf("(M2) Calculation finished in %s \n",  end.Sub(start))     
  }()  

  go func() {
    defer wg.Done()
    start := time.Now()
    fmt.Println("-----Fibonacci Recursivo-----(M3)")
    go fibonacciRecur(numero)
    //fmt.Println("Resultado Factorial recursivo:", )    
    end := time.Now()
    fmt.Printf("(M3) Calculation finished in %s \n",  end.Sub(start))     
  }()
